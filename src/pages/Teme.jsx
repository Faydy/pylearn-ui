...
export default function Teme() {
  ...
  if (!user) {
    return (
      <div className="flex h-full flex-col"><TopHeader title="Teme" /><div className="flex flex-1 items-center justify-center p-6"><div className="max-w-md rounded-2xl border border-border bg-ink p-8 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-accent/10 text-accent"><ClipboardList className="h-6 w-6" /></div><h1 className="mt-5 text-2xl font-bold text-text-main">Temele tale te așteaptă</h1><p className="mt-3 text-muted">Intră în cont pentru a vedea temele primite și progresul tău.</p><Link to="/login" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-3 font-bold text-ink transition-colors hover:bg-accent/90"><LogIn className="h-4 w-4" />Intră în cont</Link></div></div></div>
    );
  }

  return (
    <div className="flex h-full flex-col"><TopHeader title="Teme" /><main className="flex-1 overflow-y-auto p-6"><div className="mx-auto w-full max-w-7xl pb-10"><div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-accent">{teacherView ? 'Spațiul profesorului' : 'Spațiul elevului'}</p><h1 className="mt-2 text-3xl font-bold text-text-main">{teacherView ? 'Temele claselor mele' : 'Temele mele'}</h1><p className="mt-2 text-muted">{teacherView ? 'Creează, publică și urmărește progresul elevilor.' : 'Urmărește progresul și continuă de unde ai rămas.'}</p></div>{teacherView && <Link to="/teme/noua" className="inline-flex items-center justify-center gap-2 rounded-xl bg-accent px-4 py-3 font-bold text-ink transition-colors hover:bg-accent/90"><Plus className="h-5 w-5" />Creează temă</Link>}</div>

      {error ? <div className="flex items-start gap-3 rounded-2xl border border-hard/20 bg-hard/10 p-5 text-hard"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-bold">Temele nu au putut fi încărcate.</p><p className="mt-1 text-sm">{error}</p></div></div> : assignments.length === 0 ? <div className="rounded-2xl border border-dashed border-border bg-ink p-10 text-center"><ClipboardList className="mx-auto h-8 w-8 text-muted" /><h2 className="mt-4 text-xl font-bold text-text-main">{teacherView ? 'Nu ai creat încă nicio temă.' : 'Nu ai nicio temă momentan.'}</h2><p className="mt-2 text-sm text-muted">{teacherView ? 'Creează prima temă pentru una dintre clasele tale.' : 'Temele publicate de profesorii claselor tale vor apărea aici.'}</p>{teacherView && <Link to="/teme/noua" className="mt-5 inline-flex items-center gap-2 font-bold text-accent hover:text-text-main"><Plus className="h-4 w-4" />Creează temă</Link>}</div> : <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{assignments.map((assignment) => <AssignmentCard key={assignment.id} assignment={assignment} teacherView={teacherView} />)}</div>}
    </div></main></div>
  );
}
