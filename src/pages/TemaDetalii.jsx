...
export default function TemaDetalii() {
  ...
  if (!user) {
    return <div className="flex h-full flex-col"><TopHeader title="Teme" /><div className="flex flex-1 items-center justify-center p-6"><div className="max-w-md rounded-2xl border border-border bg-ink p-8 text-center"><ClipboardList className="mx-auto h-8 w-8 text-accent" /><h1 className="mt-4 text-xl font-bold text-text-main">Intră în cont pentru a vedea tema</h1><Link to="/login" className="mt-5 inline-flex font-bold text-accent hover:text-text-main">Intră în cont</Link></div></div></div>;
  }

  if (error || !assignment) {
    return <div className="flex h-full flex-col"><TopHeader title="Teme" /><div className="p-6"><div className="mx-auto max-w-3xl rounded-2xl border border-hard/20 bg-hard/10 p-6 text-hard"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5" /><div><p className="font-bold">Tema nu a putut fi afișată.</p><p className="mt-1 text-sm">{error || 'Tema nu a fost găsită.'}</p></div></div><Link to="/teme" className="mt-4 inline-flex font-bold underline">Înapoi la teme</Link></div></div></div>;
  }

  return (
    <div className="flex h-full flex-col"><TopHeader title="Teme" /><main className="flex-1 overflow-y-auto p-6"><div className="mx-auto w-full max-w-5xl pb-10"><Link to="/teme" className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-muted transition-colors hover:text-text-main"><ArrowLeft className="h-4 w-4" />Înapoi la teme</Link>
      <section className="rounded-2xl border border-border bg-ink p-6 sm:p-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2 text-sm font-bold text-accent"><ClipboardList className="h-5 w-5" />{assignment.classroomName}</div><h1 className="mt-3 text-3xl font-bold text-text-main">{assignment.title}</h1>{assignment.description && <p className="mt-3 max-w-3xl whitespace-pre-wrap text-muted">{assignment.description}</p>}</div>{teacherView && <div className="flex shrink-0 gap-2"><Link to={`/teme/${assignment.id}/edit`} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm font-bold text-text-main transition-colors hover:border-accent hover:text-accent"><Pencil className="h-4 w-4" />Editează</Link><button type="button" onClick={handleDelete} disabled={deleting} className="inline-flex items-center gap-2 rounded-xl border border-hard/30 px-3 py-2 text-sm font-bold text-hard transition-colors hover:bg-hard/10 disabled:cursor-not-allowed disabled:opacity-50">{deleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}Șterge</button></div>}</div>
      ...
      {teacherView && <section className="mt-6 rounded-2xl border border-border bg-ink p-6"><div className="mb-5 flex items-center gap-2"><Users className="h-5 w-5 text-accent" /><div><h2 className="text-xl font-bold text-text-main">Progres elevi</h2><p className="mt-1 text-sm text-muted">Progres calculat din problemele rezolvate, fără request-uri per elev.</p></div></div>{studentProgress.length === 0 ? <p className="rounded-xl border border-dashed border-border p-5 text-center text-sm text-muted">Nu există elevi în această clasă momentan.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[520px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted"><tr><th className="px-3 py-3 font-bold">Elev</th><th className="px-3 py-3 font-bold">Progres</th><th className="px-3 py-3 font-bold">Status</th></tr></thead><tbody>{studentProgress.map((student) => <tr key={student.id} className="border-b border-border last:border-b-0"><td className="px-3 py-4 font-bold text-text-main">{student.username}</td><td className="px-3 py-4 text-muted">{student.solvedCount} / {student.total} <span className="ml-1 text-xs">({student.percentage}%)</span></td><td className="px-3 py-4"><span className={`rounded-md px-2 py-1 text-xs font-bold ${student.status.tone === 'easy' ? 'bg-easy/10 text-easy' : student.status.tone === 'hard' ? 'bg-hard/10 text-hard' : student.status.tone === 'medium' ? 'bg-medium/10 text-medium' : 'bg-background text-muted'}`}>{student.status.label}</span></td></tr>)}</tbody></table></div>}</section>}
    </div></main></div>
  );
}
