...
export default function EditorTema() {
  ...
  if (!user || !teacherView) {
    return <div className="flex h-full flex-col"><TopHeader title="Teme" /><div className="flex flex-1 items-center justify-center p-4 sm:p-6"><div className="max-w-md rounded-2xl border border-border bg-ink p-6 text-center sm:p-8"><AlertTriangle className="mx-auto h-8 w-8 text-hard" /><h1 className="mt-4 text-xl font-bold text-text-main">Acces rezervat profesorilor</h1><p className="mt-2 text-sm text-muted">Doar profesorii pot crea și modifica teme.</p><Link to="/teme" className="mt-5 inline-flex font-bold text-accent hover:text-text-main">Înapoi la teme</Link></div></div></div>;
  }

  if (message.type === 'error' && !assignment && editing) {
    return <div className="flex h-full flex-col"><TopHeader title="Teme" /><div className="p-4 sm:p-6"><div className="mx-auto max-w-3xl rounded-2xl border border-hard/20 bg-hard/10 p-4 text-hard sm:p-6"><p className="font-bold">Tema nu a putut fi deschisă.</p><p className="mt-2 text-sm">{message.text}</p><Link to="/teme" className="mt-4 inline-flex font-bold underline">Înapoi la teme</Link></div></div></div>;
  }
