...
export default function DetaliiClasa() {
  ...
  if (!user) {
    return <div className="flex h-full flex-col"><TopHeader title="Clase" /><div className="flex flex-1 items-center justify-center p-6"><div className="max-w-md rounded-2xl border border-border bg-ink p-8 text-center"><DoorOpen className="mx-auto h-9 w-9 text-accent" /><h1 className="mt-4 text-xl font-bold text-text-main">Intră în cont pentru a vedea clasa</h1><Link to="/login" className="mt-5 inline-flex font-bold text-accent transition-colors hover:text-text-main">Intră în cont</Link></div></div></div>;
  }

  if (error || !classroom) {
    return <div className="flex h-full flex-col"><TopHeader title="Clase" /><main className="p-6"><div className="mx-auto max-w-3xl rounded-2xl border border-hard/20 bg-hard/10 p-6 text-hard"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-bold">Clasa nu a putut fi afișată.</p><p className="mt-1 text-sm">{error || 'Clasa nu a fost găsită.'}</p></div></div><Link to="/clase" className="mt-4 inline-flex font-bold underline">Înapoi la clase</Link></div></main></div>;
  }
