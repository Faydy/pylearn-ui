...
export default function TemaElevDetalii() {
  ...
  return (
    <div className="flex h-full flex-col"><TopHeader title="Teme" /><main className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="mx-auto w-full max-w-5xl pb-10"><Link to={`/teme/${assignmentId}`} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-muted transition-colors hover:text-text-main"><ArrowLeft className="h-4 w-4" />Înapoi la tema clasei</Link>
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-ink"><div className="border-b border-border px-4 py-5 sm:px-6"><h2 className="text-xl font-bold text-text-main">Problemele temei</h2><p className="mt-1 text-sm text-muted">Soluția afișată este cea mai recentă trimitere acceptată.</p></div><div className="divide-y divide-border">{details.map((problem) => <article key={problem.problem_id} className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:p-5"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${problem.solved ? 'bg-easy/10 text-easy' : 'bg-background text-muted'}`}>{problem.solved ? <CheckCircle2 className="h-5 w-5" /> : <Circle className="h-5 w-5" />}</div><div className="min-w-0 flex-1"><p className="font-bold text-text-main">{problem.problem_title}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-xs"><span className={`rounded-md border px-2 py-0.5 font-bold ${getDifficultyClasses(problem.problem_difficulty)}`}>{problem.problem_difficulty || 'Mixt'}</span><span className="inline-flex items-center gap-1 font-bold text-accent"><Star className="h-3.5 w-3.5 fill-current" />+{problem.problem_xp_reward || 0} XP</span><span className={problem.solved ? 'font-bold text-easy' : 'text-muted'}>{problem.solved ? (problem.submission_id ? 'Acceptată' : 'Rezolvată') : 'Nerezolvată'}</span></div></div>{problem.submission_id && problem.source_code ? <Link to={`/teme/${assignmentId}/elev/${studentId}/problema/${problem.problem_id}`} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-accent/30 bg-accent/10 px-3 py-2 text-sm font-bold text-accent transition-colors hover:bg-accent/20"><Eye className="h-4 w-4" />Vezi soluția</Link> : problem.solved ? <span className="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-muted"><Code2 className="h-4 w-4" />Fără soluție acceptată</span> : null}</article>)}</div></section>
    </div></main></div>
  );
}
