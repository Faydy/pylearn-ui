-- Public profile submission metadata is exposed through this scoped RPC. It
-- deliberately never grants direct SELECT access to public.submissions.
-- Source code is returned only when the viewer owns it, has solved the same
-- problem, or is the teacher of a class containing the submission author.
...
begin
  ...
  select
    ...
    can_view_code,
    case
      when can_view_code then coalesce(
