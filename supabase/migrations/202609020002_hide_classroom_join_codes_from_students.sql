-- Class members can read the classroom itself, but the reusable join code is
-- intentionally returned only to the class owner.
...
begin
  ...
  select
    ...
    c.teacher_id::uuid,
    p.username::text,
    case when c.teacher_id = auth.uid() then c.join_code::text else null::text end,
